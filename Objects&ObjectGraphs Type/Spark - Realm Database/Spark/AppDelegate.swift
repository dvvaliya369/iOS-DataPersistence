...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        preloadEmployees(realm: defaultRealm)
        ...
        // Pass on the default Realm
        let firstVC = navigationController.viewControllers[0] as! InnovationIdeasViewController
        firstVC.defaultRealm = defaultRealm
        
        return true
    }

    func preloadEmployees(realm: Realm) {
        let employeesAlreadySeeded = realm.objects(Employee.self).count > 0
